...
Future<List<FavFoodReceiptModel>?> getFavReceiptByIdUser(
    {required BuildContext context, required String idUser}) async {
  var client = http.Client();
  var uri = Uri.parse(
      'https://hlrvqhqntrrqjdbcbqxr.supabase.co/rest/v1/fav_receipt?select=*,food_receipt(*)&apikey=$apiKey&id_user=eq.$idUser');
  try {
    var respone =
        await client.get(uri, headers: {'Authorization': 'Bearer $bearer'});
    if (respone.statusCode == 200) {
      var json = respone.body;
      return favFoodReceiptModelFromJson(json);
    }
  } catch (e) {
...
Future<List<FavFoodReceiptModel>?> deleteFavReceiptById(
    {required int id, required BuildContext context}) async {
  var client = http.Client();
  var uri = Uri.parse(
      'https://hlrvqhqntrrqjdbcbqxr.supabase.co/rest/v1/fav_receipt?id=eq.$id&apikey=$apiKey');
  try {
    var respone =
...
Future<http.Response?> addFavReceiptData(
    FavFoodReceiptModel data, BuildContext context) async {
  http.Response? respone;
  try {
    respone = await http.post(
        Uri.parse(
            'https://hlrvqhqntrrqjdbcbqxr.supabase.co/rest/v1/fav_receipt?apikey=$apiKey&on_conflict=unique_key'),
        headers: {
